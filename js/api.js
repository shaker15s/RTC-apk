...
(function (w) {
  ...
  function isMutatingRpc(name) {
    return MUTATING_RPCS.indexOf(name) !== -1;
  }
  ...
  function isNetworkError(err) {
    ...
    if (
      ...
      msg.indexOf('انقطاع') !== -1 ||
      msg.indexOf('تعذر الاتصال') !== -1 ||
      err.status === 0 ||
  ...
  function clearRetryQueue() {
    try {
      w.localStorage.removeItem(RETRY_QUEUE_KEY);
    } catch (e) {}
  }
  ...
  function enqueueRequest(type, action, name, args, err) {
    ...
    try {
      w.dispatchEvent(new CustomEvent('rtc:request-queued', { detail: { item: item, queueLength: queue.length } }));
    } catch (e) {}
  ...
  if (typeof w.addEventListener === 'function') {
    w.addEventListener('online', function () {
      setTimeout(flushRetryQueue, 300);
    });
    if (typeof w.document !== 'undefined' && w.document.addEventListener) {
      w.document.addEventListener('visibilitychange', function () {
        if (w.document.visibilityState === 'visible') {
          setTimeout(flushRetryQueue, 500);
        }
      });
    }
  }
  ...
  w.RTCApi = {
    ...
    /* Offline Retry Queue API */
    flushRetryQueue: flushRetryQueue,
    getRetryQueue: getRetryQueue,
