...
const localStorageShim = {
  getItem(k) { return k in _storageMap ? _storageMap[k] : null; },
  setItem(k, v) { _storageMap[k] = String(v); },
  removeItem(k) { delete _storageMap[k]; },
  clear() { Object.keys(_storageMap).forEach(k => delete _storageMap[k]); }
};
const windowShim = {
...
(async function main() {
  ...
  console.log('\n[6] Retry queue — طابور إعادة المحاولة عند انقطاع الشبكة:');
  RTCApi.clearRetryQueue();
  check('الطابور يبدأ فارغاً', RTCApi.getRetryQueue().length === 0);

  /* محاكاة فشل استدعاء RPC لطفرة بسبب انقطاع الاتصال */
  vm.runInContext(`
    window.supabaseClient.rpc = async function (name, args) {
      if (name === 'student_check_in') {
        var netErr = new TypeError('Failed to fetch');
        throw netErr;
      }
  ...
  try {
    await RTCApi.checkIn('ATT-1234');
  } catch (err) {
    checkInError = err;
  }
  check('استدعاء checkIn يرمي خطأ الشبكة', checkInError !== null && RTCApi.isNetworkError(checkInError));
